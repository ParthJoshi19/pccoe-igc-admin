...
const defaultRubrics: RubricItem[] = [
  {
    criterion: "Implementation progress",
    score: 0,
    maxScore: 10,
    comments: "",
  },
  ...
  {
    criterion: "Video clarity and communication",
    score: 0,
    maxScore: 10,
    comments: "",
  },
...
function computeRubricTotals(items: RubricItem[]) {
  ...
  const sumScore = safe.reduce(
    (a, r) => a + Math.max(0, Number(r.score || 0)),
    0
  );
  const sumMax = safe.reduce(
    (a, r) => a + Math.max(1, Number(r.maxScore || 1)),
    0
  );
...
export function JudgeDashboard() {
  ...
  useEffect(() => {
    const run = async () => {
      if (!user) return;
      const judge =
        (user as any)?.username ?? (user as any)?.email ?? (user as any)?.id;
      if (!judge) return;
      ...
      try {
        ...
        const derived: TeamEvaluation[] = (data as any[]).map((t) => {
          const latestRubric =
            Array.isArray(t.rubrics) && t.rubrics.length > 0
              ? t.rubrics[t.rubrics.length - 1]
              : null;
          return {
            teamId: t.teamId,
            ...
            evaluatedAt: latestRubric?.evaluatedAt
              ? new Date(latestRubric.evaluatedAt)
              : undefined,
          };
        });
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="mt-2 text-xs text-gray-500">
            If you're unable to see the preview, please download the file. This
            issue may occur due to a mismatch between your browser version and
            the PPT format.
          </p>
        </CardContent>
      </Card>
  ...
  const handleStartEvaluation = (team: Team) => {
    ...
    const hasRubrics =
      Array.isArray((team as any)?.rubrics) &&
      ((team as any)?.rubrics as RubricItem[]).length > 0;
    ...
    const initEval: TeamEvaluation = existingEvaluation || {
      teamId: team.teamId,
      status: "Pending",
      feedback: "",
  ...
  const handleSaveEvaluation = async () => {
    ...
    try {
      ...
      const finalScore =
        currentRubrics.length > 0 ? finalFromRubrics : currentEvaluation.rating;
      ...
      const updatedEvaluations = evaluations.filter(
        (e) => e.teamId !== currentEvaluation.teamId
      );
      const latestRubric = (data?.rubrics || [])[
        Math.max(0, (data?.rubrics || []).length - 1)
      ];
      setEvaluations([
        ...updatedEvaluations,
        {
          ...
          status:
            (data?.status as TeamEvaluation["status"]) ||
            currentEvaluation.status,
          feedback: latestRubric?.comments || currentEvaluation.feedback,
          rating:
            typeof data?.finalScore === "number" ? data.finalScore : finalScore,
          evaluatedAt: latestRubric?.evaluatedAt
            ? new Date(latestRubric.evaluatedAt)
            : new Date(),
        },
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Welcome back, {user?.name}. Review and evaluate your assigned team
            submissions.
          </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Review team submissions, watch videos, and provide evaluations
          </CardDescription>
        </CardHeader>
        <CardContent>
          {teams && teams.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">
                Contact your admin to get team assignments.
              </p>
            </div>
          ) : (
      ...
      <Dialog open={evaluationDialog} onOpenChange={setEvaluationDialog}>
        <DialogContent className="sm:max-w-[720px] lg:max-w-[900px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Provide your evaluation and feedback for this team submission.
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="space-y-3">
                  ...
                  {currentRubrics.map((item, idx) => (
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setCurrentEvaluation((prev) => ({
                            ...prev,
                            rating: computeRatingFromRubrics(next),
                          }));
                        }}
                        placeholder="Score"
                      />
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          const next = currentRubrics.filter(
                            (_, i) => i !== idx
                          );
                          setCurrentRubrics(next);
                          setCurrentEvaluation((prev) => ({
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-muted-foreground">
                    Total: {computeRubricTotals(currentRubrics).sumScore}/
                    {computeRubricTotals(currentRubrics).sumMax} • Derived
                    rating: {computeRatingFromRubrics(currentRubrics)}/5
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="Can be thought">
                        Can be thought
                      </SelectItem>
                    </SelectContent>
                  </Select>
