...
import { UserPlus, Trash2, Edit, Eye, CheckCircle, Clock, Users, FileText, PlayCircle } from "lucide-react"
...
export function AdminDashboard() {
      ...
      try {
        ...
        if (data.teams && Array.isArray(data.teams)) {
          const mappedTeams: Team[] = data.teams.map((apiTeam: any) => ({
            ...
            assignedJudgeId:
              apiTeam.allocatedJudgeId !== "000000000000000000000000" ? apiTeam.allocatedJudgeId : undefined,
            presentationPPT: apiTeam.presentationPPT,
            idCardsPDF: apiTeam.idCardsPDF,
            instituteNOC: apiTeam.instituteNOC,
          }))
          setTeams(mappedTeams)
          setTotal(data?.pagination?.total ?? data?.total ?? (Array.isArray(data.teams) ? data.teams.length : 0))
          ...
          setTeams(mappedTeams)
          setTotal(data?.pagination?.total ?? (Array.isArray(data.data) ? data.data.length : 0))
        }
      } catch (error) {
  ...
  const handleAssignJudge = async (teamInternalId: string, judgeId: string) => {
    ...
    setTeams((ts) => ts.map((t) => (t._id === teamInternalId ? { ...t, assignedJudgeId: judgeId, updatedAt: new Date() } : t)))
            ...
            <CardContent>
              <div className="flex items-center justify-between mb-4">
                <div className="text-sm text-muted-foreground">
                  {totalTeamsCount > 0 ? `Showing ${pageStart}-${pageEnd} of ${totalTeamsCount}` : "No results"}
                </div>
              ...
              <div className="space-y-4">
                {teams.map((team) => {
                  ...
                  const pptUrl: string | undefined =
                    ...
                    (typeof t?.presentationPPT === "string" ? t.presentationPPT : undefined) ||
                    t?.pptUrl
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        {pptUrl && (
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => window.open(pptUrl, "_blank", "noopener,noreferrer")}
                            aria-label="View PPT"
                            title="View PPT"
                          >
                ...
                {teams.length === 0 && (
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">Team submissions will appear here once available.</p>
                  </div>
                )}
              </div>
            </CardContent>
      ...
      >
        <DialogContent className="min-w-screen w-screen h-full  overflow-hidden flex flex-col">
          <DialogHeader className="flex-shrink-0">
            <DialogTitle className="text-2xl">{selectedTeam?.teamName || "Team Details"}</DialogTitle>
            <DialogDescription>Complete team information and submission previews</DialogDescription>
          </DialogHeader>
            ...
            (() => {
              ...
              return (
                <Tabs defaultValue="info" className="flex-1 overflow-hidden flex flex-col">
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-base font-mono">{selectedTeam.teamId}</p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className="text-base">{selectedTeam.submittedAt.toLocaleString()}</p>
                          </div>
                          <div>
                      ...
                      <Card>
                        ...
                        <CardContent className="space-y-3">
                          ...
                          <div>
                            ...
                            <p className="text-base">{selectedTeam.leaderEmail || "N/A"}</p>
                          </div>
                        </CardContent>
                      </Card>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {t?.idCardsPDF && (
                              <Button
                                ...
                                onClick={() =>
                                  window.open(
                                    typeof t.idCardsPDF === "string" ? t.idCardsPDF : t.idCardsPDF.fileUrl,
                                    "_blank",
                                    "noopener,noreferrer",
                                  )
                                }
                            ...
                            {t?.instituteNOC && (
                              <Button
                                ...
                                onClick={() =>
                                  window.open(
                                    typeof t.instituteNOC === "string" ? t.instituteNOC : t.instituteNOC.fileUrl,
                                    "_blank",
                                    "noopener,noreferrer",
                                  )
                                }
                            ...
                            {!pptUrl && !videoUrl && !t?.idCardsPDF && !t?.instituteNOC && (
                  ...
                  <TabsContent value="previews" className="flex-1 overflow-auto mt-4">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-full">
                      ...
                      <Card className="flex flex-col">
                        <CardHeader className="flex-shrink-0">
                          <div className="flex items-center justify-between">
                            <CardTitle className="text-lg">Presentation PPT</CardTitle>
                            {pptUrl && (
                              <Button
                                variant="outline"
                                size="sm"
                        ...
                        <CardContent className="flex-1 flex items-center justify-center">
                          {pptViewerUrl ? (
                            ...
                            <div className="text-center space-y-4">
                              ...
                              <div>
                                <p className="text-sm text-muted-foreground mb-2">PPT preview not available</p>
                                <Button onClick={() => window.open(pptUrl!, "_blank", "noopener,noreferrer")}>
                                  Download PPT
                                </Button>
                              </div>
                            </div>
